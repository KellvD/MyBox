...
private let cornerRandius:CGFloat = 4.0;
private let imageWidth_Height:CGFloat = 36
private let labelwidth = UIScreen.main.bounds.width-60
private let textFont = UIFont.systemFont(ofSize: 14)
private let keyWindow = UIApplication.shared.keyWindow!

private let CDIdentify = "CDdentifier"
private enum CDHUDType{
    case success
    case error
    case info
    case loading
...
public class CDHUD: UIView {
    private var delay:TimeInterval = delayTime
    private var imageView:UIImageView?
    private var activityView:UIActivityIndicatorView?
    private var type:CDHUDType?
    private var text:String?
    private var selfWidth:CGFloat = 90
    private var selfHeight:CGFloat = 90
    
    private init(text:String?,type:CDHUDType,delay:TimeInterval,enable:Bool = true,offset:CGPoint = CGPoint(x: 0, y: -50)){
        self.delay = delay
        self.text = text
        self.type = type
    ...
    private func setupUI() {
        ...
        self.layer.cornerRadius = cornerRandius
        ...
        switch type {
        case .success:
            addImageView(image: (CDHUDImage.imageOfSuccess))
        case .error:
            addImageView(image: (CDHUDImage.imageOfError))
        case .info:
            addImageView(image: (CDHUDImage.imageOfInfo))
        case .loading:
            addActivityView()
        case .text:
            break
    ...
    private func generalConstraint(at view:UIView){
        view.addConstraint(width: imageWidth_Height, height: imageWidth_Height)
        if let _ = text {
            addConstraint(toCenterX: view, toCenterY: nil)
            addConstraint(with: view, topView: nil, leftView: nil, bottomView: nil, rightView: nil, edgeInset: UIEdgeInsets(top: padding, left: 0, bottom: 0, right: 0))
        }else{
    ...
    public static func showSuccess(text:String?, delay:TimeInterval,enable:Bool = true){
        CDHUD(text: text, type: .success, delay: delay,enable:enable).show()
    }
    public static func showError(text:String?,delay:TimeInterval,enable:Bool = true) {
        CDHUD(text: text, type: .error, delay: delay,enable:enable).show()
    }
    
    public static func showLoading(enable:Bool = false) {
        CDHUD(text: nil,type:.loading,delay: 0,enable:enable).show()
    }
    
    public static func showLoading(text:String?,enable:Bool = true) {
        CDHUD(text: text,type:.loading,delay: 0,enable:enable).show()
    }
    
    public static func showInfo(text:String?,enable:Bool = true) {
        CDHUD(text: text, type: .info, delay: 2.0,enable:enable).show()
    }
    
    public static func showText(text:String?,enable:Bool = true) {
        CDHUD(text: text,type:.text,delay: 2.0,enable:enable).show()
    }
    
    public func show(){
        self.animate(hide: false){
            if self.delay > 0 {
                CDHUD.asyncAfter(duration: self.delay, completion: {
                    self.hide()
                })
            }
        }
    ...
    public func hide(delay:TimeInterval = 1.5) {
        CDHUD.asyncAfter(duration: delay) {
            self.hide()
        }
    }
    ...
    public static func hide() {
        for view in keyWindow.subviews {
            ...
            if view.restorationIdentifier == CDIdentify {
                view.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var screenView:UIView = {
        ...
        $0.restorationIdentifier = CDIdentify
...
extension UIView{
    fileprivate func addConstraint(width:CGFloat,height:CGFloat){
        if width>0 {
            addConstraint(NSLayoutConstraint(item: self, attribute: .width, relatedBy: .equal, toItem: nil, attribute: NSLayoutConstraint.Attribute(rawValue: 0)!, multiplier: 1, constant: width))
        }
        if height>0 {
            addConstraint(NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: NSLayoutConstraint.Attribute(rawValue: 0)!, multiplier: 1, constant: width))
        }
    }
    ...
    func addConstraint(toCenterX xView:UIView?,
        ...
        if let xView = xView {
            addConstraint(NSLayoutConstraint(item: xView, attribute: .centerX, relatedBy: .equal, toItem: self, attribute: .centerX, multiplier: 1, constant: constantx))
        }

        if let yView = yView {
            addConstraint(NSLayoutConstraint(item: yView, attribute: .centerY, relatedBy: .equal, toItem: self, attribute: .centerY, multiplier: 1, constant: constanty))
        }
    }
    ...
    fileprivate func addConstraint(with view:UIView,
        ...
        if let topView = topView {
            addConstraint(NSLayoutConstraint(item: view, attribute: .top, relatedBy: .equal, toItem: topView, attribute: .top, multiplier: 1, constant: edgeInset.top))
        }
        if let bottomView = bottomView {
            addConstraint(NSLayoutConstraint(item: view, attribute: .bottom, relatedBy: .equal, toItem: bottomView, attribute: .bottom, multiplier: 1, constant: edgeInset.bottom))
        }
        if let leftView = leftView {
            addConstraint(NSLayoutConstraint(item: view, attribute: .left, relatedBy: .equal, toItem: leftView, attribute: .left, multiplier: 1, constant: edgeInset.left))
        }
        if let rightView = rightView {
            addConstraint(NSLayoutConstraint(item: view, attribute: .right, relatedBy: .equal, toItem: rightView, attribute: .right, multiplier: 1, constant: edgeInset.right))
        }
