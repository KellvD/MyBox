    ...
    func handleAccessoryOption(item: Int) {
        UIView.animate(withDuration: 0.25, animations: {
            ...
            if item == 0 { // 调换背景
                ...
                let scanAction = UIAlertAction(title: "扫描文稿".localize, style: .default, handler: { (_) in

                })
                scanAction.setValue(UIColor.orange, forKey: "titleTextColor")
                alert.addAction(scanAction)

                let cameraAction = UIAlertAction(title: "拍照录像".localize, style: .default, handler: { (_) in

                })
                cameraAction.setValue(UIColor.orange, forKey: "titleTextColor")
                alert.addAction(cameraAction)

                let libraryAction = UIAlertAction(title: "照片图库".localize, style: .destructive, handler: { (_) in
