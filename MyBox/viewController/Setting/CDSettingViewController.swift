...
class CDSettingViewController: CDBaseAllViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let logPrewPath = IndexPath(row: 1, section: 2)
        self.tableview.reloadRows(at: [waterPath,logPath,logPrewPath], with: .none)
    }
    ...
    lazy var optionArr: [[CDSettingOption]] = {
        let arr = [[CDSettingOption.ChangePwd,CDSettingOption.TouchID,CDSettingOption.FakeSet],
                   [CDSettingOption.WaterSet],
                   [CDSettingOption.LogSet,CDSettingOption.LogPreview]]
        return arr
    }()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if option == .LogPreview {
            cell.optionLabel.textColor = CDSignalTon.shared.logbBean.isOn ? TextBlackColor : TextLightBlackColor
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        } else if option == .LogPreview {
            if CDSignalTon.shared.logbBean.isOn {
                let logVC = CDLogPreviewViewController()
                logVC.hidesBottomBarWhenPushed = true
                self.navigationController?.pushViewController(logVC, animated: true)
            }
        }
...
class SwitchCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        optionLabel = UILabel(frame:CGRect(x: 15, y: 9, width: 100, height: 30))
        optionLabel.textColor = TextBlackColor
        optionLabel.font = TextMidFont
        optionLabel.lineBreakMode = .byTruncatingMiddle
